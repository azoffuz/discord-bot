  ...
  async execute(guild) {
    ...
    if (allowedGuilds.length > 0) {
      if (!allowedGuilds.includes(guild.id)) {
        log.warn(`[XAVFSIZLIK] Bot ruxsatsiz serverga (${guild.name} | ${guild.id}) qo'shildi. Avtomatik chiqib ketilmoqda...`);
        try {
          await guild.leave();
        } catch (err) {
          log.error(`Serverdan chiqishda xatolik (${guild.name}):`, err.message);
        }
      }
    }
  }
